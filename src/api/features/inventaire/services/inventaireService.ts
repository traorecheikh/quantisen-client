...
export class InventaireService {
  ...
  static async recordMouvementAjustement(
    movementData: CreateMouvementAjustementRequest
  ): Promise<void> {
  ...
  static async getMovementsByUser(userId: number): Promise<Mouvement[]> {
    return await cachedGet<Mouvement[]>(`${this.BASE_PATH}/utilisateur/${userId}`, {
      ttl: CACHE_TTL.MOVEMENTS,
    })
  }

  static async getAllMovements(): Promise<Mouvement[]> {
    return await cachedGet<Mouvement[]>(this.BASE_PATH, { ttl: CACHE_TTL.MOVEMENTS })
  }

  static async getAllValidLots(): Promise<Lot[]> {
    return await cachedGet<Lot[]>(`${this.BASE_PATH}/lots/valid`, { ttl: CACHE_TTL.LOTS })
  }

  static async getAllLots(): Promise<Lot[]> {
    return await cachedGet<Lot[]>(`${this.BASE_PATH}/lots`, { ttl: CACHE_TTL.LOTS })
  }

  static async getAllLotsByBoissonId(boissonId: number): Promise<Lot[]> {
    return await cachedGet<Lot[]>(`${this.BASE_PATH}/lots/boisson/${boissonId}`, {
      ttl: CACHE_TTL.LOTS,
    })
  }

  static async getAllMouvementsByLotId(lotId: number): Promise<Mouvement[]> {
    return await cachedGet<Mouvement[]>(`${this.BASE_PATH}/lots/${lotId}/mouvements`, {
      ttl: CACHE_TTL.MOVEMENTS,
    })
  }
